 ...
 *
 *<p>This class eliminates the need for de Bruijn indices or string-based variable names by
 * assigning each instance a globally unique numeric identifier. It is analogous to the
 * {@code Value} class but operates at the type level rather than the value level.
 *
 *<p>Type variables are created automatically with unique indices via a global counter. They can
 * be tracked within logical scopes using {@link TypeVarScope} for cleanup or analysis purposes.
 */
public class TypeVar {
   ...
   *
   *<p>This class implements {@link AutoCloseable} to support try-with-resources patterns for
   * automatic scope management. When a scope is open, all newly created {@link TypeVar}
   * instances are automatically registered with it.
   *
   *<p>Use {@link TypeVar#addScope()} to create and register a new scope, and either call
  ...
  public static final class TypeVarScope implements AutoCloseable {
     ...
     *
     * @return an unmodifiable list containing all {@link TypeVar} instances registered with
     *     this scope
     */
     ...
     *
     *<p>After closing, newly created type variables will no longer be registered with this
     * scope. This method is idempotent and safe to call multiple times.
     *
     * @throws Exception never thrown; present to satisfy the {@link AutoCloseable} interface
     */
    @Override
   ...
   *
   *<p>The identifier is assigned from a global counter, ensuring uniqueness across all type
   * variables. The new variable is automatically registered with all currently open scopes
   * (see {@link #addScope()}).
   */
  public TypeVar() {
   ...
   *
   *<p>The format is "t" followed by the unique numeric index (e.g., "t0", "t42").
   *
   ...
   *
   *<p>All {@link TypeVar} instances created after this call and before the scope is closed
   * will be automatically registered with the returned scope. This enables tracking of type
   * variables introduced during specific operations (e.g., type inference for an expression).
   *
   ...
   *
   *<p>After removal, newly created type variables will no longer be registered with the
